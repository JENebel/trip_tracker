...
use trip_tracker_lib::{comms::{HandshakeMessage, MacProvider, SIGNATURE_SIZE}, track_point::ENCODED_LENGTH};

use crate::{info, service, services::{modem::{self, modem_service::{self, ATResponse}}, storage_service}, ActorControl, Configuration, ExclusiveService, ModemService, Service, StorageService};

use super::{mac_provider::{self, EmbeddedMacProvider}, upload_status::{self, SessionUploadStatus, UploadStatus}};
...
impl UploadService {
    ...
    async fn setup_network(&self) {
        ...
        let config = self.storage_service.lock().await.read_config();
...
async fn upload_actor(
    ...
    loop {
        ...
        let config = storage_service.lock().await.read_config();

        loop {
            let active_session_id = storage_service.lock().await.get_local_session_id();

            // Start by uploading old unfinished session data
            for session in upload_status.lock().await.clone().sessions.iter() {
                let track_points = storage_service.lock().await.get_session_track_point_count(session.local_id);
                let missing = track_points - session.uploaded;
...
async fn upload_data(
    ...
    while missing > 0 {
        ...
        // Sign
        let signature = mac_provider.lock().await.sign(&data, &config.auth_key);
...
async fn connect(modem: &mut ModemService, connect_strategy: ConnectStrategy, config: &Configuration, mac_provider: &mut EmbeddedMacProvider) -> i64 {
    ...
    if let ConnectStrategy::Connect(_) = connect_strategy {
    ...
    let ConnectStrategy::Reconnect(session_id) = connect_strategy else {
        panic!();
    };
